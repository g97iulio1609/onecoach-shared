...
export function getWorkoutProgramDay(
  ...
  return days.find((d: unknown) => d.dayNumber === dayNumber) || null;
}
...
export function getWorkoutProgramDayByWeek(
  ...
  const week = program.weeks?.find((w: unknown) => w.weekNumber === weekNumber);
  if (!week) {
    return null;
  }
  return week.days?.find((d: unknown) => d.dayNumber === dayNumber) || null;
}
...
export function getWorkoutProgramWeek(
  ...
  return program.weeks?.find((w: unknown) => w.weekNumber === weekNumber) || null;
}
