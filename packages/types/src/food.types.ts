...
export interface FoodItem {
  ...
  macrosPer100g: Macros; // Standardizzato per 100g (REQUIRED)
  servingSize: number; // Grammi per porzione standard (REQUIRED)
  unit: string; // Default "g" (REQUIRED)
  imageUrl?: string; // Image URL (Optional)
  brandId?: string; // Brand reference (Optional)
  mainMacro: MainMacro; // Predominant macronutrient (REQUIRED)
  proteinPct: number; // Protein percentage (REQUIRED)
  carbPct: number; // Carbs percentage (REQUIRED)
  fatPct: number; // Fat percentage (REQUIRED)
  metadata?: {
    // Optional metadata
    category?: string;
    [key: string]: unknown;
...
export interface FoodItemTranslation {
  id: string;
  foodItemId: string;
  locale: string; // e.g., 'en', 'it', 'es'
  name: string; // Translated name (REQUIRED)
  description: string; // Translated description (REQUIRED)
  createdAt: string;
  updatedAt: string;
}
...
export interface DishSegmentationResult {
  items: Array<{
    name: string;
    quantity: number; // grammi stimati
