...
export interface CopilotCapabilities {
  canGenerate: boolean; // Can generate new plans/programs
  canModify: boolean; // Can modify existing data
  canAnalyze: boolean; // Can analyze data
  canChat: boolean; // General chat capability
  canUseCamera: boolean; // Food camera integration
  canAccessHealth: boolean; // Health data integration
 ...
 */
export type ScreenContextBuilder = (
  userId: string,
  params?: Record<string, string | number>
) => Promise<{
