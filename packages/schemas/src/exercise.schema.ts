...
// URL opzionale: accetta stringa vuota, null, undefined, o URL valido
...
const stringArraySchema = z.array(z.string().trim().min(1).max(128));
...
export const createExerciseSchema = baseExercisePayloadSchema.superRefine((value, ctx) => {
  const locales = value.translations.map((translation: any) => translation.locale.toLowerCase());
...
export const updateExerciseSchema = updateExerciseBaseSchema
  ...
  .superRefine((value, ctx) => {
    ...
    if (value.translations) {
      const locales = value.translations.map((translation: any) =>
